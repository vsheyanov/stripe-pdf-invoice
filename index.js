...
function StripePdfInvoice() {
    if (!(this instanceof StripePdfInvoice)) {
        return new StripePdfInvoice();
    }
};
...
StripePdfInvoice.prototype.generate = function(invoice, config, callback) {
  ...
  invoice.label_invoice_by = invoice.label_invoice_by || 'invoice fromOcto Lab Ltd';
  ...
  invoice.pdf_name = 'INVOICE_' + moment.unix(invoice.date).format('YYYY-MM-DD') + '_#' + invoice.number + '.pdf';
  ...
  callback(invoice.pdf_name, wkhtmltopdf(html, { pageSize: 'letter' }));
};
